...
exports.getAllTweets = (callback) => {
    db.query("SELECT tweet.*, user.username FROM tweet JOIN user ON tweet.id = user.id;", (error, result) => {
        if (error) {
    ...
    exports.deleteTweet = (tweetID, callback) => { // params = tweet = obj.id callback is a func
        // callback = function
    ...
    exports.editTweet = (userID, tweetText, callback) => { // params = tweet = obj.id callback is a func
        // callback = function
        ...
        db.query(`UPDATE tweet SET text = "${tweetText}" WHERE id = ${userID};`, (error, result) => {
            // db.query(`DELETE FROM tweet WHERE id = ${tweet.id};`, (error, result) => {
            if (error) {
                console.log("error: ", error);
                callback(error, null);
